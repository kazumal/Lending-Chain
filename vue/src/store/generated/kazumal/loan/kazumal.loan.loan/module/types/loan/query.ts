...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("kazumal.loan.loan.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new Reader(data)));
  }

  Loan(request: QueryGetLoanRequest): Promise<QueryGetLoanResponse> {
    const data = QueryGetLoanRequest.encode(request).finish();
    const promise = this.rpc.request("kazumal.loan.loan.Query", "Loan", data);
